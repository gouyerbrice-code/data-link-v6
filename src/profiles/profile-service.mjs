...
export class ProfileService {
  ...
  async list() { return this.r.list("profiles"); }
  async get(id) { return this.r.find("profiles",id); }
  snapshotConfiguration({profileVersionId,ruleVersionId,configuration={}}) {
    ...
    const configurationHash=createHash("sha256").update(JSON.stringify({profile_version_id:profileVersionId??null,rule_version_id:ruleVersionId??null,configuration})).digest("hex");
