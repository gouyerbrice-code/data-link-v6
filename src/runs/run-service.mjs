...
export class RunService{
  ...
  transition(c,runId,status,patch={}){this.ctx(c);if(!RUN.includes(status))throw new Error("Invalid RUN state");const run=this.r.find("runs",runId);if(!run||run.tenant_id!==c.tenant_id)throw new DataLinkError(ERROR_CODES.NOT_FOUND,"Run not found",{status:404});const now=new Date().toISOString();return this.r.update("runs",runId,{status,...patch,...(status==="RUNNING"?{started_at:now,heartbeat_at:now}:{}),...(status==="COMPLETED"||status==="FAILED"||status==="CANCELLED"?{finished_at:now}: {})});}
  stepTransition(c,stepId,status,patch={}){this.ctx(c);if(!STEP.includes(status))throw new Error("Invalid STEP state");const s=this.r.find("steps",stepId);if(!s)throw new DataLinkError(ERROR_CODES.NOT_FOUND,"Step not found",{status:404});const run=this.r.find("runs",s.run_id);if(!run||run.tenant_id!==c.tenant_id)throw new DataLinkError(ERROR_CODES.NOT_FOUND,"Step not found",{status:404});return this.r.update("steps",stepId,{status,...patch});}
}
