...
export function similarity(a, b) {
  const left = normalizeValue(a), right = normalizeValue(b);
...
function tokenScore(a, b, tokenFrequency = new Map()) {
  const A = new Set(designationTokens(a)), B = new Set(designationTokens(b));
...
export function scorePair(left, right, { fields = {}, signalStats = {} } = {}) {
  ...
  const validA = isValidEan(barcodeA), validB = isValidEan(barcodeB);
  const barcodeExact = validA && validB && barcodeA === barcodeB;
  const barcodeConflict = validA && validB && barcodeA !== barcodeB;
  const reference = exact(A.reference, B.reference);
  const supplierReference = exact(A.supplier_reference, B.supplier_reference);
  ...
  const scores = { reference: reference ? 1 : similarity(A.reference, B.reference), barcode: barcodeExact ? 1 : (barcodeConflict ? 0 : null), supplier_reference: supplierReference ? 1 : similarity(A.supplier_reference, B.supplier_reference), designation, brand: brand ? 1 : similarity(A.brand, B.brand) };
