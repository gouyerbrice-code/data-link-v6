...
export class PipelineExecutionService {
  constructor({ repository, runService, profileService, versions }) { this.r=repository; this.runs=runService; this.profiles=profileService; this.v=versions; }
  ...
  async execute(ctx,{sourceFileId,profileVersionId=null,ruleVersionId=null,configuration={}}) {
    ...
    const sf=await this.r.find("source_files",sourceFileId);
    if(!sf||sf.tenant_id!==ctx.tenant_id) throw new DataLinkError(ERROR_CODES.NOT_FOUND,"Source file not found",{status:404});
    const snaps=(await this.r.findBy("raw_snapshots",x=>x.source_file_id===sourceFileId&&x.tenant_id===ctx.tenant_id)).sort((a,b)=>b.snapshot_version-a.snapshot_version);
    ...
    const run=await this.runs.create(ctx,{source_file_id:sourceFileId,profile_version_id:profileVersionId,rule_version_id:ruleVersionId,configuration_version:configSnap.configuration_version,configuration_hash:configSnap.configuration_hash});
    await this.runs.transition(ctx,run.id,"RUNNING",{worker:"local"});
    try {
      const rawStep=await this.runs.step(ctx,run.id,{step_type:"RAW",sequence:1,idempotency_key:`${run.execution_id}:RAW:${snap.id}`});
      await this.runs.stepTransition(ctx,rawStep.id,"RUNNING");
      await this.runs.stepTransition(ctx,rawStep.id,"COMPLETED",{progress:100,output:{snapshot_id:snap.id,record_count:snap.record_count}});

      const rows=(await this.r.list("raw_records")).filter(x=>x.snapshot_id===snap.id&&x.tenant_id===ctx.tenant_id).sort((a,b)=>a.record_number-b.record_number);
      const profileStep=await this.runs.step(ctx,run.id,{step_type:"PROFILING",sequence:2,idempotency_key:`${run.execution_id}:PROFILING:${snap.id}`});
      await this.runs.stepTransition(ctx,profileStep.id,"RUNNING");
      const prof=profileRows(rows.map(x=>x.payload));
      const pr=await this.r.insert("profiling_results",{tenant_id:ctx.tenant_id,run_id:run.id,raw_snapshot_id:snap.id,domain:prof.domain,confidence:prof.confidence,row_count:prof.row_count,column_count:prof.column_count,result:prof,created_at:new Date().toISOString()});
      await this.runs.stepTransition(ctx,profileStep.id,"COMPLETED",{progress:100,output:{profiling_result_id:pr.id,domain:prof.domain,confidence:prof.confidence}});

      const normStep=await this.runs.step(ctx,run.id,{step_type:"NORMALIZATION",sequence:3,idempotency_key:`${run.execution_id}:NORMALIZATION:${snap.id}`});
